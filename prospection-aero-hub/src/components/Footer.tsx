        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          ...
          <div>
            <div className="mb-4 flex items-center gap-3">
              <img src={logo} alt="Prospection" loading="lazy" className="h-8 w-auto brightness-0 invert" />
              <span className="w-px h-6 bg-white/20"></span>
              <img src={totalLogo} alt="TotalEnergies" loading="lazy" className="h-6 w-auto brightness-0 invert" />
            </div>
          ...
          {/* Coluna 2 — Navegação */}
          <div>
            <h3 className="font-heading text-white text-label font-semibold uppercase tracking-[0.12em] mb-4">
              Navegação
            </h3>
            <ul className="space-y-2">
              {footerLinks.map((l) => (
                <li key={l.href}>
                  <a
                    href={l.href}
                    className="text-text-muted hover:text-gold text-sm font-body transition-colors duration-200"
                  >
                    {l.label}
                  </a>
                </li>
